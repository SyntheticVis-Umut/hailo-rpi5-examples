    ...
    if user_data.use_frame and frame is not None:
        ...
        if any_preview:
            ...
            if user_data.show_boxes_only:
                ...
                # NORMAL MODE: Draw masks on full frame
                # Hardware overlay handles boxes, but we need to draw masks in Python
                for idx, detection in enumerate(detections):
                    bbox = detection.get_bbox()
                    masks = detection.get_objects_typed(hailo.HAILO_CONF_CLASS_MASK)
                    if len(masks) != 0:
                        ...
                        if x_max > x_min and y_max > y_min:
                            ...
                            mask_overlay[y_min:y_max, x_min:x_max] = (
                                (resized_mask_data[:y_max-y_min, :x_max-x_min, np.newaxis] > 0.5) * color
                            )
